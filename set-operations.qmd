...
SELECT c.short_name, ig.name AS income_group
FROM countries AS c
INNER JOIN income_groups AS ig ON c.income_group_id = ig.income_group_id
WHERE ig.name = 'Low income'
...
FROM countries AS c
INNER JOIN income_groups AS ig ON c.income_group_id = ig.income_group_id
WHERE ig.name = 'Lower middle income'
  AND NOT c.is_aggregate
...
SELECT c.short_name, r.name AS region
FROM countries AS c
...
FROM countries AS c
INNER JOIN income_groups AS ig ON c.income_group_id = ig.income_group_id
WHERE ig.name = 'Low income' AND NOT c.is_aggregate
...
FROM countries AS c
INNER JOIN income_groups AS ig ON c.income_group_id = ig.income_group_id
WHERE ig.name = 'Lower middle income' AND NOT c.is_aggregate
...
```sql
SELECT c.short_name FROM countries AS c
INNER JOIN income_groups AS ig ON c.income_group_id = ig.income_group_id
WHERE ig.name = 'Low income'
UNION
SELECT c.short_name FROM countries AS c
...
5. Write a query using `UNION ALL` that combines all high-income countries and all upper-middle-income countries into a single result set. Include `short_name` and the income group name (joined from the `income_groups` table). Sort alphabetically by `short_name`.
...
8. Using `EXCEPT`, find countries that had GDP per capita data in 1990 but have **no** GDP per capita data in 2022. Return `short_name` and the region name (joined from the `regions` table), sorted by region then `short_name`.
