...
#### Embedded databases in gaming

Another example of embedded database usage I like to highlight? Turn-based video games! Sid Meier's Civilization (aka "Civ") is a personal favorite of mine. The Civ franchise is still going strong after 35+ years, and it had an enormous influence on the formation of the "4X" genre of strategy games (eXplore, eXpand, eXploit, and eXterminate).

![Screenshot from Sid Meier's Civilization V](images/civ5_city_screen.jpg)

More recent entries in the Civ series have used an array of SQLite database files behind the scenes to store local game data, and modders are known to change these databases when developing mods. The screenshot below shows a tiny portion of the SQLite database that Civ V uses, which has roughly 370 distinct tables, containing everything from unit stats to each civilization's characteristics. The `leaders` table, for example, has columns that are used to control each leader's diplomacy, loyalty, and military aggression. The `units` table, seen here, shows the different characteristics of military and non-military units in the game.

![Screenshot of a SQL query in DataGrip, showing a selection of data from Sid Meier's Civilization V](images/civ_v_database.png)
...
Localhost databases are also pretty much useful for only one thing: local development, done by a single individual. The only way for other people to use a localhost database you have deployed to your laptop is to put your laptop on the internet and allow external connections to it. Not a smart idea.
