...
In most real situations, you won't want to include every column. Listing only the columns you need makes results easier to read and faster to retrieve from large tables.
...
::: {.callout-tip}
Relying on `SELECT *` in production code is considered poor practice because your query could break if someone adds, removes, or reorders columns in the table. Naming your columns explicitly makes queries more predictable and easier to maintain. As a rule, I tell my students that `SELECT *` is fine when you're just exploring the data as part of an ad-hoc analysis, but including it in a production system raises substantial risks, so don't do it!
:::
...
The `AS` keyword is optional in PostgreSQL and most other SQL dialects; you can write the alias directly after the column name without it. Using `AS` explicitly is recommended, however, because it makes the intent of the query immediately clear to anyone reading it.

### Combining Text Columns

PostgreSQL uses the `||` operator to concatenate (combine) strings together. This lets you build a single display value out of multiple columns, or add literal text between column values.
...
You can add as many literal strings as you like. Here, a comma and space are inserted between the last name and the birth country to produce a compact biographical label for each actor:
...
SELECT first_name, last_name, oscar_nominations, oscar_wins
FROM actors
WHERE oscar_nominations BETWEEN 5 AND 8
ORDER BY oscar_nominations DESC;
```
...
::: {.callout-warning}
You should use explicit `>=` and `<=` comparisons instead of `BETWEEN` for most situations. The inclusive behavior is easy to forget, which makes errors common, and `BETWEEN` cannot express partial ranges. The alphabetical edge cases with strings make it especially error-prone there. Meanwhile, explicit operators are unambiguous. `BETWEEN` is fine when you encounter it in existing code or reach for it with simple integer or date ranges, but it is not the preferred form for new queries.
:::
...
In a later chapter, I'll show you how to use window functions like `RANK` and `DENSE_RANK` to calculate rankings in similar situations.
...
With `%` wildcards on both sides of a search term, you can find text that occurs anywhere in a column. The following query finds actors whose birth names contain the name "Mary," regardless of whether it's a first or middle name:
...
This is one aspect of SQL that many people find frustrating, but it's a natural byproduct of how relational databases operate. As you will learn in later chapters, though, there are strategies we can take when writing complex SQL queries to avoid having to duplicate the same logic repeatedly.

Note, however, that `ORDER BY` does not have the same limitation. Because `ORDER BY` is evaluated later in the logical order, it can reference aliases defined in `SELECT`. That is why `ORDER BY height_in` works in the query above even though `WHERE height_in` does not.
