...
`BETWEEN` is **inclusive on both ends**: a row where `birth_date` equals either boundary date is included in the result. This makes it reliable for discrete types like integers and dates, where the boundaries are exact values.
...
::: {.callout-warning}
Prefer explicit `>=` and `<=` comparisons over `BETWEEN` for most situations. The inclusive behavior is easy to forget, which makes off-by-one errors common, and `BETWEEN` cannot express half-open ranges (for example, "1950 up to but not including 1970"). Explicit operators are unambiguous. `BETWEEN` is fine when you encounter it in existing code or reach for it with simple integer or date ranges, but it is not the preferred form for new queries.
:::
