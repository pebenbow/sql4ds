...
Many database IDEs also show a formatted role list, with attributes and group memberships, in their own roles or users panel.
...
Some database IDEs display a table's access control list (ACL) directly in a permissions or security panel. For a portable view that works the same regardless of which IDE you use, query `information_schema`:
