...
### Labeling Subtotals with COALESCE
...
Wrapping each grouping column in `COALESCE` replaces the NULL subtotal markers with a readable label without changing any of the counts:
...
The per-country subtotal rows now show `All Genres` in the `primary_genre` column, and the grand total row shows `Grand Total` in `birth_country`. The data is unchanged; only the presentation differs.
